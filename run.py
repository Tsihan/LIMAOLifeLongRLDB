...
def DbmsxExecuteSql(sql_str, comment=None, timeout_ms=None, remote=True, repeat=1):
    raise NotImplementedError
...
def ParseExecutionResult(
    ...
    messages.append(
        "  {:.1f} (predicted {:.1f})  {}".format(
            query_node.cost, query_node.info["curr_predicted_latency"], expert_hint_str
        )
    )
    if found_plans:
        ...
        for p_cost, found in zip(predicted_costs, found_plans):
            ...
            found_hint_str_physical = HintStr(
                found_plan, with_physical_hints=True, engine=engine
            )
            extras = [
...
def InitializeModel(
    p, model, sim, soft_assign_tau=0.0, soft_assign_use_ema=False, ema_source_tm1=None
):
        ...
        for layer in model.out_mlp:
            if isinstance(layer, nn.Linear):
                ...
                def _Add(w):
                    ...
                    w.add_(
                        torch.normal(
                            mean=0.0, std=p.param_noise, size=w.shape, device=w.device
                        )
                    )
...
class BalsaModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        if p.adamw:
            optimizer = torch.optim.AdamW(
                self.parameters(), lr=self.learning_rate, weight_decay=p.adamw
            )
        else:
    ...
    def on_after_backward(self):
        if self.global_step % 10 == 0:
            ...
            total_norm = (
                torch.stack([torch.norm(param) for param in self.parameters()])
                .sum()
                .detach()
            )
...
class BalsaAgent(object):
    ...
    def __init__(self, params):
        ...
        try:
            ...
            if "dictionary changed size during iteration" not in str(e):
                raise e
            print(
                "Connected to ray but ray.available_resources() failed, "
        ...
        if p.linear_decay_to_zero:
            self.adaptive_lr_schedule = train_utils.AdaptiveMetricPiecewiseDecayToZero(
                [(0, p.lr)],
    ...
    def Cleanup(self, signum, frame):
        ...
        print(
            "Received signal {}; calling ray.shutdown().".format(
    ...
    def _MakeWorkload(self, is_origin=False):
        ...
        # qihan: here we change the workload on the fly
            ...
            if is_origin:
                with open('data/TPCH10_assorted/initial_policy_data.pkl', "rb") as f:
                    workload = pickle.load(f)
            # Filter queries based on the current query_glob.
                workload.FilterQueries(
                    'queries/tpch_assorted', ['*.sql'], ['3a.sql', '10d.sql', '8a.sql', '8f.sql', '8b.sql', '13i.sql'])
            else:

                with open('data/TPCH10_assorted_2/initial_policy_data.pkl', "rb") as f:
                    workload = pickle.load(f)
            # Filter queries based on the current query_glob.
                workload.FilterQueries(
                    'queries/tpch_assorted_2', ['*.sql'], ['7c.sql', '14e.sql', '12i.sql', '14h.sql', '7a.sql', '7f.sql'])
    ...
    def _InitLogging(self):
        ...
        self.loggers = [
            pl_loggers.TensorBoardLogger(
                save_dir=os.getcwd(), version=None, name="tensorboard_logs"
            ),
        ...
        with open(
            os.path.join(self.wandb_logger.experiment.dir, "params.txt"), "w"
        ) as f:
    ...
    def _MakeTrainer(self, train_loader):
        ...
        return pl.Trainer(
            ...
            early_stop_callback=pl.callbacks.EarlyStopping(
                patience=p.validate_early_stop_patience, mode="min", verbose=True
            ),
    ...
    def SelectPlan(
        ...
        if p.epsilon_greedy > 0:
            ...
            for i, (_, plan) in enumerate(found_plans):
                hint_str = HintStr(
                    plan, with_physical_hints=p.plan_physical, engine=p.engine
                )
    ...
    def PlanAndExecute_episode(self, model, planner, is_test=False, max_retries=3):
        ...
        if  self.is_origin_workload:
            p.init_experience = 'data/TPCH10_assorted/initial_policy_data.pkl'
            p.test_query_glob = ['3a.sql', '10d.sql', '8a.sql', '8f.sql', '8b.sql', '13i.sql']
            p.query_dir = 'queries/tpch_assorted'
        else:
            p.init_experience = 'data/TPCH10_assorted_2/initial_policy_data.pkl'
            p.test_query_glob = ['7c.sql', '14e.sql', '12i.sql', '14h.sql', '7a.sql', '7f.sql']
    ...
    def PlanAndExecute(self, model, planner, is_test=False, max_retries=3):
        ...
        for i, node in enumerate(nodes):
            ...
            self.LogScalars(
                [
                    (
                        "predicted_latency_expert_plans/q{}".format(
                            node.info["query_name"]
                        ),
            ...
            hint_str = HintStr(
                found_plan, with_physical_hints=p.plan_physical, engine=p.engine
            )
            hinted_plan = found_plan
            ...
            if is_test:
                ...
                curr_timeout = self.timeout_controller.GetTimeout(node)
            print(
                "q{},(predicted {:.1f}),{}".format(
                    node.info["query_name"], predicted_latency, hint_str
                )
            )
            to_execute.append(
                (
        ...
        self.timer.Stop("plan_test_set" if is_test else "plan")
        self.timer.Start(
            "wait_for_executions_test_set" if is_test else "wait_for_executions"
        )
        self.wandb_logger.log_metrics(
            {
                "train/position-of-min-predicted-cost-plan": wandb.Histogram(
        ...
        print(
            "{}Waiting on Ray tasks...value_iter={}".format(
    ...
    def SaveBestPlans(self):
        ...
        for query_name, value_tup in sorted(self.best_plans._cache.items()):
            ...
            hint = HintStr(
                best_node, with_physical_hints=p.plan_physical, engine=p.engine
            )
        ...
        pd.DataFrame({"query": qnames, "latency_ms": latencies}).to_csv(
            os.path.join(best_plans_dir, "latencies.txt"), header=True, index=False
        )
        w.save(os.path.join(best_plans_dir, "latencies.txt"), base_path=wandb_dir)
        # plans.pkl.
    ...
    def UpdateMovingAverage(self, model, moving_average, ema_decay=None):
        ...
        moving_average_state_dict = self.moving_average_model_state_dict[moving_average]
