...
class AttentionMerger(nn.Module):
    ...
    def forward(self, conv_outputs):
        ...
        # Shape: [3, batch size, feature_dim]
    ...
    # 首先调整weights的形状以匹配stacked_outputs的broadcasting要求
        # Adjusted shape for broadcasting: [3, 1, 1]
        weights = weights.view(-1, 1, 1)
        # Broadcasting weights to each feature_dim
        weighted_outputs = weights * stacked_outputs

    # 加权求和，不增加额外的维度，保持[batch size, feature_dim]形状
        # Shape: [batch size, feature_dim]
...
def _batch(data):
    ...
    xs = np.zeros((len(data), np.max(lens),
                  data[0].shape[1]), dtype=np.float32)
...
def get_last_preorder_index(curr, root_index=1):
    ...
    last_index_right = get_last_preorder_index(
        curr.children[1], last_index_left + 1)
...
def _make_indexes(root):
    ...
    preorder_ids, _ = _make_preorder_ids_tree(
        root, hash_join_set, nested_loop_join_set)
    vecs = []
    _walk(preorder_ids, vecs)
    vecs = np.asarray(vecs).reshape(-1, 1)
    ...
    vecs_nested_loop_join = np.where(
        nested_loop_join_mask, node_ids, 0).reshape(-1, 1)

    # Update vecs to include only nodes that are not part of hash join or nested loop subtrees
    vecs = np.where(~hash_join_mask & ~nested_loop_join_mask,
                    node_ids, 0).reshape(-1, 1)
...
def _featurize_tree(curr_node, node_featurizer):
    ...
    ret_nested_loop_join = np.zeros(
        (num_nodes + 1, vec_size), dtype=np.float32)

    ret[1:] = vecs
    ret_hash_join[1:len(vecs_hash_join) + 1] = vecs_hash_join
    ret_nested_loop_join[1:len(vecs_nested_loop_join) +
                         1] = vecs_nested_loop_join
...
def make_and_featurize_trees(trees, node_featurizer):
    ...
    hash_join_indexes_batch = torch.from_numpy(
        _batch(hash_join_indexes_list)).long()
    nested_loop_join_indexes_batch = torch.from_numpy(
        _batch(nested_loop_join_indexes_list)).long()

    # 批量化特征数据并转换为浮点数张量
    trees_batch = torch.from_numpy(_batch(trees_list)).transpose(1, 2).float()

    hash_join_trees_batch = torch.from_numpy(
        _batch(hash_join_trees_list)).transpose(1, 2).float()
    nested_loop_join_trees_batch = torch.from_numpy(
        _batch(nested_loop_join_trees_list)).transpose(1, 2).float()
