...
print("Read", len(init_queries), "queries from", init_query_directory)
...
for query_directory in query_directory_list:
    ...
    print("Read", len(queries), "queries from", query_directory)
    if "imdb_" in query_directory:
        pg_connection_str = PG_CONNECTION_STR_1
        timeout = TIME_OUT_IMDB
    elif "tpch_" in query_directory:
        pg_connection_str = PG_CONNECTION_STR_2
